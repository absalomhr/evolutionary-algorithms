...
def decode_half_chromosome(chromosome):
    global a,b
...
def f(x):
    div_point = len(x) // 2
    xv = decode_half_chromosome(x[:div_point])
    yv = decode_half_chromosome(x[div_point:])
    part1 = math.exp(-0.2 * math.sqrt(0.5*((xv**2)+(yv**2))))
    part2 = math.exp(0.5*(math.cos(2*math.pi*xv)+math.cos(2*math.pi*yv)))
    return -20 * part1 - part2 + math.exp(1) + 20
...
def nextgeneration():
    ...
    print( "Best solution so far, generation: "  + str(n_gen))
    if n_gen % 10 == 0:
        print(F0)
    mid = len(F0[0]) // 2
    print("Chromosome: " + str(F0[0]))
    print( "f("+str(decode_half_chromosome(F0[0][:mid]))+", "+ str(decode_half_chromosome(F0[0][mid:])) +")= "+
           str(f(F0[0])) )
